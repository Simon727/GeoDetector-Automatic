...
class GeoDetectorProcessor:
    ...
    def class_paths(self):
        ...
        if not paths:
            raise ValueError(f"五级分类目录中没有 *_5class.tif 文件: {self.class_dir}")
        return paths
    ...
    def run_extract(self):
        self.ensure_dirs()
        out_csv = self.output_dir / "渔网点XY提取值.csv"
        if out_csv.exists() and self.skip_existing:
            self.log(f"跳过已有渔网提取表: {out_csv.name}")
            return out_csv, None

        self.log("开始制作渔网并提取 Y 与 X 值。")
        grid = self.target_grid()
        transform = grid["transform"]
        y, y_valid = self.read_y()
        factor_arrays = self.load_class_arrays()
        common_valid = self.common_valid_mask(y_valid, factor_arrays)
        rows, cols = np.where(common_valid)
        total = rows.size
        if total == 0:
            raise ValueError("Y 与 X 因子没有共同有效的网格点。")

        names = list(factor_arrays.keys())
        header = ["row", "col", "x", "y", "Y"] + names
        chunk = 200_000
        with open(out_csv, "w", newline="", encoding="utf-8-sig") as fh:
            ...
            for start in range(0, total, chunk):
                ...
                block = [
                    ...
                    ys,
                    y[r, c],
                    *[factor_arrays[name][r, c] for name in names],
                ]
                writer.writerows(zip(*block))
                self.log(f"渔网提取进度: {end:,}/{total:,}")

        self.log(f"渔网提取完成，共 {total:,} 个有效点，输出: {out_csv}")
        return out_csv, int(total)
    ...
    def run_detector(self):
        ...
        factor_arrays = self.load_class_arrays()
        common_valid = self.common_valid_mask(y_valid, factor_arrays)
        common_n = int(common_valid.sum())
        y_n = int(y_valid.sum())
        if common_n == 0:
            raise ValueError("Y 与 X 因子没有共同有效样点，无法运行地理探测器。")

        rows = []
        q_values = {}
        for name, arr in factor_arrays.items():
            q, n, strata = q_stat(y, arr, common_valid)
            q_values[name] = q
        ...
        for i, name_i in enumerate(names):
            for j, name_j in enumerate(names):
                if i == j:
                    ...
                    combo = factor_arrays[name_i].astype("uint16") * 10 + factor_arrays[name_j].astype("uint16")
                    q12, _, _ = q_stat(y, combo, common_valid)
        ...
        with open(summary_path, "w", encoding="utf-8") as fh:
            fh.write(f"目标鱼网/采样网格: {self.y_tif.name} cell centers, {self.target_grid()['crs']}\n")
            fh.write(f"网格尺寸: {self.target_grid()['width']} x {self.target_grid()['height']}\n")
            fh.write(f"边界内且 Y 有效的网格点数: {y_n}\n")
            fh.write(f"Y 与 X 因子均有效的网格点数: {common_n}\n")
