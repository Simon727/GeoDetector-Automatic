...
DEFAULT_ROOT = Path("/Users/simon/Desktop/YW/Smallthings/Geo260")
DEFAULT_FACTOR_DIR = DEFAULT_ROOT / "因子下载"
DEFAULT_SHP = DEFAULT_ROOT / "长江流域" / "长江流域.shp"
DEFAULT_Y_TIF = DEFAULT_ROOT / "Y" / "NPP.tif"
DEFAULT_CLIP_DIR = DEFAULT_ROOT / "因子裁剪后"
DEFAULT_CLASS_DIR = DEFAULT_ROOT / "五级分类后"
DEFAULT_OUTPUT_DIR = DEFAULT_ROOT / "Output"

CLIP_NODATA = -9999.0
CLASS_NODATA = 0
DEFAULT_JENKS_SAMPLE_LIMIT = 100_000
DEFAULT_SAMPLE_INTERVAL_M = 1000
DEFAULT_INPUT_POINT_CRS = "EPSG:4326"
RANDOM_SEED = 260
...
class GeoDetectorProcessor:
    def __init__(self, config, log):
        ...
        self.input_points_path = Path(config["input_points_path"]) if config.get("input_points_path") else None
        self.input_points_crs = config.get("input_points_crs") or DEFAULT_INPUT_POINT_CRS
    ...
    def input_points_output_path(self):
        if self.input_points_path is None:
            return self.output_dir / "输入点XY提取值.csv"
        stem = clean_filename(self.input_points_path.stem)
        return self.output_dir / f"输入点XY提取值_{stem}.csv"
    ...
    def detect_xy_columns(self, df):
        ...
        raise ValueError("CSV 点文件中未找到坐标列，请使用 x/y、lon/lat、longitude/latitude 或 经度/纬度。")

    def point_attributes(self, df, excluded_cols, reserved_cols):
        attrs = df.drop(columns=[col for col in excluded_cols if col in df.columns], errors="ignore").copy()
        rename_map = {}
        used = set(reserved_cols)
        for col in attrs.columns:
            new_col = str(col)
            if new_col in used:
                new_col = f"原始_{new_col}"
            while new_col in used:
                new_col = f"{new_col}_1"
            used.add(new_col)
            rename_map[col] = new_col
        return attrs.rename(columns=rename_map)

    def read_input_points(self):
        if self.input_points_path is None or not str(self.input_points_path).strip():
            raise ValueError("请先选择输入点文件。")
        if not self.input_points_path.exists():
            raise FileNotFoundError(f"输入点文件不存在: {self.input_points_path}")

        target_crs = self.target_grid()["crs"]
        suffix = self.input_points_path.suffix.lower()
        if suffix in {".csv", ".txt", ".tsv"}:
            sep = "\t" if suffix == ".tsv" else None
            if sep is None:
                df = pd.read_csv(self.input_points_path)
            else:
                df = pd.read_csv(self.input_points_path, sep=sep)
            x_col, y_col = self.detect_xy_columns(df)
            xs_raw = pd.to_numeric(df[x_col], errors="coerce").to_numpy(dtype="float64")
            ys_raw = pd.to_numeric(df[y_col], errors="coerce").to_numpy(dtype="float64")
            valid_xy = np.isfinite(xs_raw) & np.isfinite(ys_raw)
            dropped = int((~valid_xy).sum())
            if dropped:
                self.log(f"输入点 CSV 坐标为空或异常，预剔除 {dropped:,} 行。")
            df = df.loc[valid_xy].reset_index(drop=True)
            xs_raw = xs_raw[valid_xy]
            ys_raw = ys_raw[valid_xy]
            source_crs = self.input_points_crs
            if str(source_crs) == str(target_crs):
                xs, ys = xs_raw, ys_raw
            else:
                xs_list, ys_list = transform_coords(source_crs, target_crs, xs_raw.tolist(), ys_raw.tolist())
                xs = np.asarray(xs_list, dtype="float64")
                ys = np.asarray(ys_list, dtype="float64")
            attrs = self.point_attributes(df, [x_col, y_col], {"point_id", "x", "y", "Y"})
            self.log(f"读取 CSV 输入点 {len(df):,} 个，坐标系按 {source_crs} 处理。")
            return pd.DataFrame({"point_id": np.arange(1, len(df) + 1), "x": xs, "y": ys}), attrs

        gdf = gpd.read_file(self.input_points_path)
        if gdf.empty:
            raise ValueError("输入点文件为空。")
        if gdf.crs is None:
            self.log(f"输入点文件没有 CRS，按用户填写的 {self.input_points_crs} 处理。")
            gdf = gdf.set_crs(self.input_points_crs)
        if gdf.crs != target_crs:
            gdf = gdf.to_crs(target_crs)

        gdf = gdf[~gdf.geometry.is_empty & gdf.geometry.notna()].copy()
        if gdf.empty:
            raise ValueError("输入点文件没有可用几何。")
        point_mask = gdf.geometry.geom_type.isin(["Point"])
        if not bool(point_mask.all()):
            dropped = int((~point_mask).sum())
            self.log(f"输入矢量中有 {dropped:,} 个非 Point 几何，已剔除。")
            gdf = gdf.loc[point_mask].copy()
        if gdf.empty:
            raise ValueError("剔除非 Point 几何后没有剩余输入点。")

        xs = gdf.geometry.x.to_numpy(dtype="float64")
        ys = gdf.geometry.y.to_numpy(dtype="float64")
        attrs = self.point_attributes(gdf.drop(columns="geometry"), [], {"point_id", "x", "y", "Y"})
        attrs = attrs.reset_index(drop=True)
        self.log(f"读取矢量输入点 {len(gdf):,} 个，坐标系: {gdf.crs}")
        return pd.DataFrame({"point_id": np.arange(1, len(gdf) + 1), "x": xs, "y": ys}), attrs

    def extract_values_for_points(self, points_df, attrs_df):
        grid = self.target_grid()
        xs = points_df["x"].to_numpy(dtype="float64")
        ys = points_df["y"].to_numpy(dtype="float64")
        ...
        with rasterio.open(self.y_tif) as src:
            y_data = src.read(1).astype("float64")
            y_nodata = src.nodata
        y_values = np.full(len(points_df), np.nan, dtype="float64")
        y_values[in_grid] = y_data[raster_rows[in_grid], raster_cols[in_grid]]
        valid = in_grid & self.valid_y_mask(y_values, y_nodata)
        ...
        for name, arr in factor_arrays.items():
            ...
            valid &= np.isfinite(values) & (values >= 1) & (values <= self.n_classes)
        ...
        if attrs_df is not None and not attrs_df.empty:
            result = pd.concat([result, attrs_df.reset_index(drop=True)], axis=1)

        before = len(result)
        result = result.loc[valid].copy()
        dropped = before - len(result)
        if result.empty:
            raise ValueError("输入点提取后全部为空值或异常值，没有剩余有效行。")
        self.log(f"输入点提取完成，保留 {len(result):,} 行，剔除 {dropped:,} 行。")
        return result, dropped

    def run_input_points_extract(self):
        self.ensure_dirs()
        out_csv = self.input_points_output_path()
        if out_csv.exists() and self.skip_existing:
            self.log(f"跳过已有输入点提取表: {out_csv.name}")
            return out_csv, None
        self.log("开始按用户输入点提取 Y 与 X 值。")
        points_df, attrs_df = self.read_input_points()
        result, dropped = self.extract_values_for_points(points_df, attrs_df)
        result.to_csv(out_csv, index=False, encoding="utf-8-sig")
        self.log(f"输入点提取表已输出: {out_csv}")
        return out_csv, int(len(result)), int(dropped)
    ...
    def save_heatmap(self, q_matrix, out_path):
        ...
        ax.set_title("地理探测器交互探测 q 值热力图", fontproperties=font, pad=14)
        ...
        cbar.ax.set_ylabel("q value", rotation=270, labelpad=12)
        fig.tight_layout()
...
class GeoDetectorApp(tk.Tk):
    def __init__(self):
        ...
        self.vars = {
            "factor_dir": tk.StringVar(value=str(DEFAULT_FACTOR_DIR)),
            "shp_path": tk.StringVar(value=str(DEFAULT_SHP)),
            "y_tif": tk.StringVar(value=str(DEFAULT_Y_TIF)),
            "clip_dir": tk.StringVar(value=str(DEFAULT_CLIP_DIR)),
            "class_dir": tk.StringVar(value=str(DEFAULT_CLASS_DIR)),
            ...
            "input_points_path": tk.StringVar(value=""),
            "input_points_crs": tk.StringVar(value=DEFAULT_INPUT_POINT_CRS),
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="地理探测器一条龙 V3.0 输入点版", style="Title.TLabel").pack(side="left")
        ...
        self._path_row(
            ...
            "file",
            [
                ("Point files", "*.csv *.txt *.tsv *.shp *.geojson *.json *.gpkg"),
                ("CSV", "*.csv *.txt *.tsv"),
        ...
        options.grid(row=7, column=0, columnspan=3, sticky="ew", padx=10, pady=(6, 10))
        ...
        heatmap_options.grid(row=8, column=0, columnspan=3, sticky="ew", padx=10, pady=(0, 10))
        ...
        ttk.Label(heatmap_options, text="输入点坐标系").pack(side="left")
        ttk.Entry(heatmap_options, textvariable=self.vars["input_points_crs"], width=14).pack(side="left", padx=(8, 22))
        ...
        buttons = [
            ...
            ("输入点提取 X/Y", self.on_input_points_extract),
    ...
    def choose_path(self, var_name, mode, filetypes=None):
        ...
        initial = str(Path(current).parent if mode == "file" else Path(current))
        if mode == "dir":
            value = filedialog.askdirectory(initialdir=initial)
        else:
            value = filedialog.askopenfilename(initialdir=initial, filetypes=filetypes or [("All files", "*")])
        if value:
    ...
    def on_input_points_extract(self):
        self.run_task("按输入点提取 X/Y 值", lambda p: p.run_input_points_extract())
